...
int main(int argc, char* argv[])
{
    ...
    if (!ProgramSettings.cipherkey.empty()) {
        ...
        for (const auto& elem : ProgramSettings.cipherkey) {
            if (!std::isdigit(elem)) {
                std::cerr
                    << "[error] cipher key must be an unsigned long integer for Caesar cipher,\n"
                    << "        the supplied key (" << ProgramSettings.cipherkey
                    << ") could not be successfully converted" << std::endl;
                return 1;
            }
        }
        caesarKey = std::stoul(ProgramSettings.cipherkey);
    }

    // Run the Caesar cipher (using the specified key and encrypt/decrypt flag) on the input text
    std::string outputText{runCaesarCipher(inputText, caesarKey, ProgramSettings.encrypt)};
